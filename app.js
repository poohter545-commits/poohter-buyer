...
function orderStatusLabel(status = "") {
  ...
  return {
    ...
    packed: "Your order is ready",
    shipped: "Out from warehouse",
    out_for_delivery: "Out from warehouse",
...
function orderStatusClass(status = "") {
  if (["delivered", "successful"].includes(status)) return "status-delivered";
  if (["accepted", "packed", "out_from_warehouse", "out_for_delivery", "shipped"].includes(status)) return "status-ready";
  if (status === "cancelled") return "status-cancelled";
  return "status-pending";
}

function normalizePakistaniMobileNumber(value = "") {
...
function validateCheckoutPhone() {
  const normalized = normalizePakistaniMobileNumber(state.user?.phone);
  if (!normalized) {
    notify("Enter a valid Pakistani mobile number before checkout: 03XXXXXXXXX, +923XXXXXXXXX, or 923XXXXXXXXX.", "error");
    return false;
  }
  if (state.user.phone !== normalized) {
    state.user = { ...state.user, phone: normalized };
...
function absoluteAssetUrl(value = "") {
  ...
  if (/^https?:/i.test(path)) {
    try {
      ...
      if (/\/api\/media$/i.test(url.pathname)) return path;
    } catch {
      return path;
    }
  }
  return `${API_URL}/media?src=${encodeURIComponent(path)}`;
}
...
function mountPoohterContactWidget() {
  ...
  const accountType = mount.dataset.accountType || window.POOHTER_ACCOUNT_TYPE || "buyer";
  ...
  mount.innerHTML = `
    <style>
      .poohter-contact-widget{position:fixed;right:18px;bottom:18px;z-index:80;display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap;justify-content:flex-end}
      .poohter-contact-widget button,.poohter-contact-widget a{border:0;border-radius:8px;min-height:44px;padding:0 14px;font:inherit;font-weight:900;text-decoration:none;display:inline-flex;align-items:center;gap:8px;box-shadow:0 12px 24px rgba(15,23,42,.18);cursor:pointer}
      .poohter-contact-widget .call-btn{background:#0f172a;color:#fff}
      .poohter-contact-widget .wa-btn{background:#16a34a;color:#fff}
      .poohter-contact-form{position:fixed;right:18px;bottom:76px;width:min(340px,calc(100vw - 36px));background:#fff;border:1px solid #dbe3ef;border-radius:8px;padding:14px;box-shadow:0 20px 48px rgba(15,23,42,.2);display:none;gap:10px}
      .poohter-contact-form.open{display:grid}
      .poohter-contact-form input,.poohter-contact-form textarea{width:100%;border:1px solid #cbd5e1;border-radius:8px;padding:10px;font:inherit}
      .poohter-contact-form textarea{min-height:74px;resize:vertical}
      .poohter-contact-form .form-row{display:flex;gap:8px}
      @media(max-width:560px){.poohter-contact-widget{left:12px;right:12px}.poohter-contact-widget button,.poohter-contact-widget a{flex:1;justify-content:center}.poohter-contact-form{left:12px;right:12px;width:auto}}
    </style>
    <form class="poohter-contact-form">
      <input name="phone" placeholder="03XXXXXXXXX" required />
      <textarea name="message" placeholder="Message optional"></textarea>
      <div class="form-row">
        <button class="call-btn" type="submit">Send Request</button>
        <button class="call-btn" type="button" data-close-contact>Close</button>
      </div>
    </form>
    <button class="call-btn" type="button" data-open-contact>Request Call</button>
    <a class="wa-btn" href="https://wa.me/${phoneNumber}?text=${whatsappText}" target="_blank" rel="noreferrer">WhatsApp</a>
  `;
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      const response = await fetch(`${API_URL}/support/request-call`, {
        method: "POST",
      ...
      if (typeof notify === "function") notify("Call request sent.", "success");
    } catch (error) {
...
document.addEventListener("DOMContentLoaded", mountPoohterContactWidget);
...
async function confirmPlaceOrder() {
  ...
  if (!validateCheckoutPhone()) return;
...
function returnActionMarkup(order = {}) {
  const orderId = escapeHtml(order.id);
  if (order.return_status) {
    return `<div class="return-panel return-note">Return status: ${escapeHtml(order.return_status)}</div>`;
  }
  if (order.return_period_expired) {
    return '<div class="return-panel return-expired">Return period expired</div>';
  }
  if (!order.return_eligible) return "";

  return `
    <div class="return-panel">
      <button class="btn-return" data-open-return="${orderId}" type="button">Request Return</button>
      <div class="return-form hidden" id="return-form-${orderId}">
        <textarea id="return-reason-${orderId}" placeholder="Return reason"></textarea>
        <button class="btn-return" data-submit-return="${orderId}" type="button">Submit Return</button>
      </div>
    </div>
  `;
}
...
async function submitReturnRequest(orderId) {
  ...
  try {
    await api(`/orders/${encodeURIComponent(orderId)}/return-request`, "POST", { reason });
    ...
    container.innerHTML = orders.map((order) => {
      ...
      <div class="order-item-list">
          <div class="order-item-mini">
            <span>${items.length ? `${items.length} item${items.length === 1 ? "" : "s"} in this order` : "Order details protected"}</span>
            <span style="font-weight: 700">${escapeHtml(orderStatusLabel(order.status))}</span>
          </div>
...
$("#search-box").addEventListener("input", debounce(filterAndSortProducts));
...
$("#signup-form").addEventListener("submit", async (event) => {
  ...
  const normalizedPhone = normalizePakistaniMobileNumber($("#signup-phone").value);
  if (!isOtpStep && !normalizedPhone) {
    notify("Enter a valid Pakistani mobile number: 03XXXXXXXXX, +923XXXXXXXXX, or 923XXXXXXXXX.", "error");
    return;
  }
...
document.addEventListener("click", (event) => {
  ...
  if (button) {
    ...
    if (button.dataset.openReturn) {
      document.getElementById(`return-form-${button.dataset.openReturn}`)?.classList.toggle("hidden");
      return;
    }
